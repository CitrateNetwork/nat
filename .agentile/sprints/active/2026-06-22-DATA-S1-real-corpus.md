...
| WP-D5 | **BPE tokenizer** (+ model-based quality, deferred) | learned vocab behind the same `encode`/`decode`/`vocab_size` interface | ✅ **BPE done** — `nat-data::bpe::Bpe` (byte-level, deterministic, dep-free; train/encode/decode/save/load) + `nat-corpus train-bpe` CLI. On the corpus: **1.99 bytes/token @ vocab 1024**. BPE autoregressive LM: **2.37 bits/byte** vs the byte LM's 3.42 — a **~31% reduction** (more context/position). Model-based quality gate still planned. |
