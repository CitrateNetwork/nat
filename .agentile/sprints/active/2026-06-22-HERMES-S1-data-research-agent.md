...
| WP-H5 | **Source connectors** (Gutenberg, HF, permissive code) | each fetches + screens; new domains hit the approval queue | 🟡 Gutenberg + **code** done — `nat-data::gutenberg` + `nat-data::code` (walk repo, skip vendored, split files; license is a fail-closed flag) + `from-gutenberg`/`from-code`. Proven: 3 PD books + rust-lang/log (Apache-2.0) → combined **191K tokens** → held-out 4.05–4.12 bits/byte. Remaining: HF connector + a **from-text/markdown** connector (SICP CC-BY-SA, Rust Book) + the values-spine fetch list (`research-loop/READING_LIST.md`). |
